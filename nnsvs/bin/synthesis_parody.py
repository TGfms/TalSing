...
def my_app(config: DictConfig) -> None:
    ...
    timelag_config = OmegaConf.load(join(env['RUN_SCRIPT_DIR'], config.timelag.model_yaml))
    timelag_model = hydra.utils.instantiate(timelag_config.netG).to(device)
    checkpoint = torch.load(
        join(env['RUN_SCRIPT_DIR'], config.timelag.checkpoint),
        map_location=lambda storage, loc: storage,
        weights_only=False  # after PyTorch 2.6
    )
    timelag_model.load_state_dict(checkpoint["state_dict"])
    timelag_in_scaler = joblib.load(join(env['RUN_SCRIPT_DIR'], config.timelag.in_scaler_path))
    timelag_out_scaler = joblib.load(join(env['RUN_SCRIPT_DIR'], config.timelag.out_scaler_path))
    timelag_model.eval()

    # duration
    duration_config = OmegaConf.load(join(env['RUN_SCRIPT_DIR'], config.duration.model_yaml))
    duration_model = hydra.utils.instantiate(duration_config.netG).to(device)
    checkpoint = torch.load(
        join(env['RUN_SCRIPT_DIR'], config.duration.checkpoint),
        map_location=lambda storage, loc: storage,
        weights_only=False  # after PyTorch 2.6
    )
    duration_model.load_state_dict(checkpoint["state_dict"])
    duration_in_scaler = joblib.load(join(env['RUN_SCRIPT_DIR'], config.duration.in_scaler_path))
    duration_out_scaler = joblib.load(join(env['RUN_SCRIPT_DIR'], config.duration.out_scaler_path))
    duration_model.eval()

    # acoustic model
    acoustic_config = OmegaConf.load(join(env['RUN_SCRIPT_DIR'], config.acoustic.model_yaml))
    acoustic_model = hydra.utils.instantiate(acoustic_config.netG).to(device)
    checkpoint = torch.load(
        join(env['RUN_SCRIPT_DIR'], config.acoustic.checkpoint),
        map_location=lambda storage, loc: storage,
        weights_only=False  # after PyTorch 2.6
    )
    acoustic_model.load_state_dict(checkpoint["state_dict"])
    acoustic_in_scaler = joblib.load(join(env['RUN_SCRIPT_DIR'], config.acoustic.in_scaler_path))
    acoustic_out_scaler = joblib.load(join(env['RUN_SCRIPT_DIR'], config.acoustic.out_scaler_path))
    acoustic_model.eval()
    ...
    # Vocoder
    if config.vocoder.checkpoint != 'None' and len(config.vocoder.checkpoint) > 0:
        vocoder, vocoder_in_scaler, vocoder_config = load_vocoder(
            join(env['RUN_SCRIPT_DIR'], config.vocoder.checkpoint),
            device,
            acoustic_config,
        )
    else:
    ...
    binary_dict, numeric_dict = hts.load_question_set(
        join(env['RUN_SCRIPT_DIR'], config.synthesis.qst)
    )

    in_dir = join(env['RUN_SCRIPT_DIR'], config.in_dir)
    out_dir = join(env['RUN_SCRIPT_DIR'], config.out_dir)
    os.makedirs(out_dir, exist_ok=True)
    out_comp_dir = join(env['RUN_SCRIPT_DIR'], f"{config.out_dir}_comp")
    os.makedirs(out_comp_dir, exist_ok=True)
    tar_file = os.path.join(in_dir, config.file_name + '.lab')
    logger.info("Processes %s utterances...", len(tar_file))

    labels = hts.load(join(in_dir, tar_file))
