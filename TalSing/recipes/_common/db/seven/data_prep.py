...
def get_parser():
    parser = argparse.ArgumentParser(
        description="Data preparation for SEVEN", #.
        formatter_class=argparse.ArgumentDefaultsHelpFormatter,
    )
    parser.add_argument("seven_root", type=str, help="SEVEN song dir") #.
...
mono_lab_files = sorted(glob(join(args.seven_root, "**/*.lab"))) #.
muxicxml_files = sorted(glob(join(args.seven_root, "**/*.xml"))) #.
...
for mono_path, xml_path in zip(mono_lab_files, muxicxml_files):
    ...
    if len(align_mono_lab) != len(sinsy_mono_lab):
        print(f"len of align_mono_lab : {len(align_mono_lab)}, len of sinsy_mono_lab : {len(sinsy_mono_lab)}")
        import ipdb
        ipdb.set_trace()
    assert len(align_mono_lab) == len(sinsy_mono_lab)
    if (np.asarray(align_mono_lab.contexts) != np.asarray(sinsy_mono_lab.contexts)).any():
        import ipdb
        ipdb.set_trace()
    assert (
...
for lab_align_path, lab_score_path in zip(full_lab_align_files, full_lab_score_files):
    ...
    wav_path = join(args.seven_root, name, f"{name}.wav") #.
    assert wav_path
