...
if [ ${stage} -le 100 ] && [ ${stop_stage} -ge 100 ]; then
    echo "stage 100: Synthesis PRODY waveforms"
    . $NNSVS_COMMON_ROOT/synthesis_parody.sh
fi

if [ ${stage} -le 101 ] && [ ${stop_stage} -ge 101 ]; then
    echo "stage 101: Synthesis waveforms from Original Score."
    . $NNSVS_COMMON_ROOT/synthesis_fromScore.sh
fi
